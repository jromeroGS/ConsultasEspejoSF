            ...
            switch (Lista1)
                    {
                        ...
                        case "Movimiento de Producción":
                            ...
                            break;
                        case "Registro de Fidelización":
                            nombreObjeto = "Loyalty_Registry__c Reg";
                            cadenaSeleccion = "Reg.Name NúmeroRegistro, Rec.name Tipo_Registro, Reg.CUC__c ClienteUnico, Reg.Country__c País, AccountNumbre__c NúmeroCuenta, Status__c Estado, Cuenta.Name NombreCuenta, businessEmail__c Email_Comercial, emailBilling__c Email_Facturación, CallCounter__c Cant_Llamadas, CallScore__c Calif_Llamada, generalAvg__c Llamadas_Teléfono,  A_LoyaltyAction__c PercepciónGeneral, Flow1Sumary__c Comentarios, LinkGMB__c Link_GMB, Link_Pacom__c Link_Pacom, urlWeb__c Url_Web, Reg.ID ID ";

                            switch (Lista2)
                            {
                                case "Número de Registro":
                                    nombrefiltro = "Reg.Name";
                                    query2 = "= '" + Texto1 + "'";
                                    break;
                                case "Cliente Unico":
                                    nombrefiltro = "Reg.CUC__c";
                                    query2 = "= '" + Texto1 + "'";
                                    break;

                            }
                            query = conexion.getRegFidelizacion(cadenaSeleccion, nombreObjeto, nombrefiltro, query2);
                            break;
